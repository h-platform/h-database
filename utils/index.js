...
module.exports = {
  ...
  getRandomElement: function (list) {
    ...
    return list[module.exports.getRandomInt(0,max)];
  },

  getRandomElements: function (list, count) {
    var max = list.length - 1;
    var randomList = [];
    for (var i = 0; i <= (count - 1); i++){
      randomList.push(list[module.exports.getRandomInt(0,max)]);
    }
    return randomList;
  },
  ...
  unflatten: function( records, pathProperty, rootPath, childrenProperty, parent, tree ) {
      ...
      var children = _.filter( records, function(child){ return path.getParent(child[pathProperty]) == parent[pathProperty]; });
      ...
      if( !_.isEmpty( children )  ){
          ...
          _.each( children, function( child ){ path.unflatten( records, pathProperty, rootPath, childrenProperty, child ) } );
      }
